...
| `lowest_price`   | Should be greater than 0                                             |
...
| `id`                   | Must be populated and unique                               |
| `name`                 | Must be populated                                          |
| `physical_description` | Must be a list of strings delimited by semicolons          |
...
|---------------|-----------------------------------------------------|
| `id`          | Must be populated and unique                        |
| `menu_id`     | Must be defined and menu.id must exist, else `NULL` |
...
|----------------|--------------------------------------------------------------|
| `id`           | Must be populated and unique                                 |
| `menu_page_id` | Must be defined and `menu_page.id` must exist, else `NULL`   |
| `price`        | Must be greater than 0 or `NULL`                             |
| `dish_id`      | Must be defined and `dish.id` must exist, else `NULL`        |
...
SQLite database. SQL also excels at multi-row and multi-table aggregation which is beyond the capabilities of
OpenRefine.
...
Using these views, we're able to easily identify and fix all rows that violate a specific foreign key IC violation.
These views also generate aggregations across tables to re-create columns, such as `page_count`, `lowest_price`, and
`highest_price`, to name a few. We perform data cleaning through non-destructive means (i.e. creating replacement SQL
Views that represent the clean dataset) in this step. Like the previous step, this step is required for subsequent
foreign key IC violations cleaning work.
...
- `menus_appeared` and `times_appeared` were re-caclulated based on the menus/menu_items/menu_pages actually in the
  data. Thus, zero is no longer a viable result and neither are negative numbers. The semantic difference between these
  two columns remains somewhat unclear.
- `first_appeared` and `last_appeared` similarly were drawn from actual data from menus/menu_items/menu_pages but it was
  clear that not all the values contained there are accurate. The data were examined and the true range of values
...
- `lowest_price` and `highest_price` were re-caclulated based on the menus/menu_items/menu_pages. While the statistics
  above a truncated to one decimal point. The actual values _are_ greater than zero, albeit small -- 0.01 and
  0.005,respectively.
...
- `venue` were categorical values with many slight differences. These were merged as much as could reasonably be
  performed.
- `place` had its values converted to standard casing, had various delimiters normalized or removed (
  i.e. `[PHILADELPHIA, PA.]` converted to `Philadelphia, Pa`), had minor punctuation removed where appropriate, and had
  other minor inconsistencies removed and standardized.
- `physical_description` - had delimiter formatting standardized to internal semicolon (`;`) separation, with no
  trailing semicolon character. All text in this column was also converted to lowercase.
...
- `page_count` was re-calculated based on the data provided in other tables by counting foreign keys.
- `dish_count` was re-calculated based on the data provided in other tables by counting foreign keys.
...
- `price` was left unchanged.
- `high_price` was left unchanged.
- `xpos` was left unchanged.
- `ypos` was left unchanged.
...
The above table shows that we have improved referential integrity by fixing relational IC violations on 2 of the 3
...
demonstrates that data quality is improved both in terms of database referential integrity and overall integrity
constraints.
...
- Python and its CSV libraries
- Pandas
- OpenRefine server, API and home-grown client
...
- Use our Python code and home-grown OpenRefine client library to:
    ...
    - Apply single-value IC violations for all fields with IC violations, according to the policy for the field's column
      IC violations according to policy
...
- Python
- OpenRefine home-grown client, API and server
- SQLite
...
not perfect. The result of this method is a system that can process the entire NYPL restaurants dataset, but executes
relatively slowly and inefficiently.

While distributed data processing systems like Apache Spark, Google Cloud's BigQuery, and others are out of scope of
this project, we recognize such systems would be critical for processing datasets much larger than the one we've used in
this project. These systems would also have likely allowed us to process the NYPL restaurants dataset much more quickly
and efficiently.
...
learning applications. Further analysis could also be done to identify superior-quality subsets of these data for use in
other, more mission-critical applications.
